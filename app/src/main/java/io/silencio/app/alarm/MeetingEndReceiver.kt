...
class MeetingEndReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val preMeetingState = prefs.getPreMeetingDndState()
                disableDnd(context, preMeetingState)
    ...
    private fun disableDnd(context: Context, preMeetingState: Int) {
        val nm = context.getSystemService(NotificationManager::class.java)
        val audioManager = context.getSystemService(AudioManager::class.java)

        if (nm.isNotificationPolicyAccessGranted) {
            if (preMeetingState == NotificationManager.INTERRUPTION_FILTER_ALL) {
                audioManager.ringerMode = AudioManager.RINGER_MODE_NORMAL
                nm.setInterruptionFilter(NotificationManager.INTERRUPTION_FILTER_ALL)
            }
            Log.d("AlarmScheduler", "DND disabled — preMeetingState=$preMeetingState")
        } else {
